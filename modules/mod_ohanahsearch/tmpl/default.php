...
<div class="panelContent ohanah module<?php echo $params->get( 'moduleclass_sfx' ) ?>">
	...
	<form method="get" action="<?=@route('option=com_ohanah&view=events')?>" id="filterForm" class="box short">
	
		<input type="hidden" name="option" value="com_ohanah" />
		<input type="hidden" name="view" value="events" />
		<input type="hidden" name="Itemid" value="<?=KRequest::get('get.Itemid', 'int')?>" />
		<div>
			<? if ($params->get('showFilters')) : ?>
				...
				<script>
					...
					$jq(function() {
						...
						$jq('select[name="ohanah_category_id"]').change(function(){
							$jq('#filterForm').submit();
						});
			...
			<? if ($params->get('showTextSearch')) : ?>
				<div style="width:; float:left; margin-right:10px">
